<template>
  <div class="bg-white p-6 md:px-30 sm:p-8 md:p-10 rounded-2xl border border-gray-200 shadow-md space-y-8">
      <h1 class="text-2xl font-bold text-gray-900">Thông tin thanh toán</h1>
    <section class="form-section bg-white p-6 sm:p-8 md:p-10 rounded-2xl border border-gray-200 shadow-lg space-y-8 ">
      <label class="block text-lg font-bold text-gray-800 mb-4">Phương thức nhận tiền</label>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <label
            class="flex items-center p-4 rounded-lg border-2 cursor-pointer transition-all"
            :class="paymentMethod === 'qr' ? 'border-purple-500 bg-purple-50' : 'border-gray-300 bg-white hover:border-gray-400'"
        >
          <input type="radio" v-model="paymentMethod" value="qr" class="h-5 w-5 text-purple-600 focus:ring-purple-500">
          <span class="ml-3 font-semibold text-gray-800">Thanh toán qua mã QR</span>
        </label>
        <label
            class="flex items-center p-4 rounded-lg border-2 cursor-pointer transition-all"
            :class="paymentMethod === 'bank' ? 'border-purple-500 bg-purple-50' : 'border-gray-300 bg-white hover:border-gray-400'"
        >
          <input type="radio" v-model="paymentMethod" value="bank" class="h-5 w-5 text-purple-600 focus:ring-purple-500">
          <span class="ml-3 font-semibold text-gray-800">Chuyển khoản ngân hàng</span>
        </label>
      </div>


    <section v-if="paymentMethod === 'qr'">
    ...
    <section v-if="paymentMethod === 'bank'">
      ...
      <div class="space-y-4">
        <div>
          ...
          <input type="text" id="bankName" v-model="bankInfo.bankName" class="w-full p-3 bg-white border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-200 focus:border-purple-500" placeholder="Ví dụ: Ngân hàng TMCP Kỹ thương Việt Nam (Techcombank)">
        </div>
        <div>
          <label for="accountHolder" class="block text-sm font-semibold text-gray-700 mb-1">Tên chủ tài khoản</label>
          <input type="text" id="accountHolder" v-model="bankInfo.accountHolder" class="w-full p-3 bg-white border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-200 focus:border-purple-500" placeholder="Ví dụ: Nguyen Chua Quy">
        </div>
        <div>
          <label for="accountNumber" class="block text-sm font-semibold text-gray-700 mb-1">Số tài khoản</label>
          <input type="text" id="accountNumber" v-model="bankInfo.accountNumber" class="w-full p-3 bg-white border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-200 focus:border-purple-500" placeholder="Nhập số tài khoản ngân hàng của bạn">
        </div>
        <div>
          <label for="branch" class="block text-sm font-semibold text-gray-700 mb-1">Chi nhánh (tùy chọn)</label>
          <input type="text" id="branch" v-model="bankInfo.branch" class="w-full p-3 bg-white border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-200 focus:border-purple-500" placeholder="Ví dụ: Hội sở chính">
        </div>
      </div>
    </section>
    </section>
