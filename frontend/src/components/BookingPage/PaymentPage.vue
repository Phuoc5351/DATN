// PaymentPage.vue (Bản đầy đủ đã được sửa)
    ...
    <main class="container mx-auto px-4 py-8">
      ...
      <div class="mt-10 max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div class="bg-white p-6 md:p-8 rounded-lg shadow-sm border border-gray-200">
          ...
          <div v-if="orderItems.length === 0" class="text-center text-gray-500 py-8"><p>Giỏ hàng của bạn đang trống.</p><button @click="router.push({ name: 'EventDetail'})" class="mt-4 text-blue-600 hover:underline">Quay lại chọn vé</button></div>
          <div v-else class="space-y-6">
            <div v-for="item in orderItems" :key="item.id" class="flex items-center space-x-4">
              ...
              <div class="text-right"><p class="font-semibold text-gray-800">{{ formatPrice(item.price) }}</p></div>
            </div>
          </div>
        ...
        <div class="bg-white p-6 md:p-8 rounded-lg shadow-sm border border-gray-200">
          ...
          <div v-else>
            <div class="grid grid-cols-3 gap-4 mb-6">
              <label v-for="method in paymentMethods" :key="method.id" :class="['border rounded-lg p-3 flex items-center justify-center cursor-pointer transition-all', selectedPaymentMethod === method.id ? 'border-2 border-blue-600 shadow-md' : 'border-gray-300 hover:border-gray-400']"><input type="radio" name="payment_method" :value="method.id" v-model="selectedPaymentMethod" class="sr-only"><img :src="method.logo" :alt="method.name" class="h-6"></label>
            </div>
            <form @submit.prevent="handlePayment" class="space-y-4">
              <div>
                <label for="name_on_card" class="block text-sm font-medium text-gray-700">Họ và Tên</label>
                <input v-model="paymentDetails.name" type="text" id="name_on_card" placeholder="Nhập họ và tên của bạn" class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
              </div>
              <div>
                <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                <input v-model="paymentDetails.email" type="email" id="email" placeholder="Nhập email của bạn" class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
              </div>
              <div>
                <label for="phone" class="block text-sm font-medium text-gray-700">Số điện thoại</label>
                <input v-model="paymentDetails.phone" type="tel" id="phone" placeholder="Nhập số điện thoại của bạn" class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
              </div>
              ...
              <div class="flex items-center pt-4">
                ...
                <label for="agreement" class="ml-2 block text-sm text-gray-900">By clicking this, I agree to TixFlow <a href="#" class="text-blue-600 hover:underline font-medium">Privacy Policy</a>.</label>
              </div>
...
// --- STATE MANAGEMENT ---
// Sửa đổi paymentDetails để có thêm các trường cần thiết
...
// =======================================================
// SỬA ĐỔI CHÍNH: HÀM handlePayment
// =======================================================
const handlePayment = () => {
  ...
  // (Trong thực tế, bạn sẽ gọi API thanh toán ở đây và chờ kết quả)
  // Giả lập thanh toán thành công:

  // 1. Chuẩn bị dữ liệu để gửi sang trang xác nhận
  const confirmationData = {
    order: {
      productName: 'Sky tour SƠN TÙNG MTP Concert', // Lấy tên chung hoặc từ vé đầu tiên
      quantity: orderItems.value.reduce((acc, item) => acc + (item.description.match(/\d+/)?.[0] || 1), 0),
...
onMounted(() => {
  ...
  if (savedCart) {
    ...
    // Nếu không có giỏ hàng, không thể thanh toán
  }
